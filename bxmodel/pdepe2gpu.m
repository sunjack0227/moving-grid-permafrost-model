function varargout = pdepe2gpu(m,pde,ic,bc,xmesh,t,options,varargin)
...
%   $Revision: 1.8.4.10 $  $Date: 2013/10/03 01:33:25 $

% Check inputs
%[t,xmesh]=cupvsgup(0,t,xmesh );
if nargin < 7
  options = [];
  if nargin < 6
...
try
  if hasEvents
    [t,y,te,ye,ie] = ode113(@pdeodes,t,y0(:),opts);
  else
    [t,y] = ode113(@pdeodes,t,y0(:),opts);
  end
catch ME
...
function varargout=cupvsgup(csu, varargin )
%csu:0 cup to gpu；1：gup to cpu
%   此处显示详细说明
if csu==0
for k = 1:length(varargin)
%eval([inputname(k) '=gpuArray(varargin{k});'])
