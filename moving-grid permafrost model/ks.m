...
%D1=D(:,:,1);
L=load([pwd '\parameter\' 'tu（modis 8天10年).mat']);
tu=L.tu;
L=load([pwd '\parameter\' 'x(20,40).mat']);
x1=L.x1;
x2=L.x2;
i1=length(x1);
i2=length(x2);
...
t1=0:365*ceil(max(tu/365));
 ...
 YC.cs=load([pwd '\parameter\' '唐古拉.mat']);
...
%      sol=sol(end-365+1:end,:);
     ...
     sol=bxpdes(x1,t,100,0,ys,pd,yc,pq,ku,dwf); 
     ys=sol(end,:);
    ys(i1:i2)=pq*x2(i1:i2)+ys(i1)-pq*x2(i1);
     pd=4;
     [solc,xx] =bxpdesc(x2,t1,[],x2,ys,pd,yc,pq,ku,dwf);
